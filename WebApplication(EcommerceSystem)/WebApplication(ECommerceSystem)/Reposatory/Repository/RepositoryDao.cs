...
namespace Reposatory.Repository
{
    class RepositoryDao
    {
        ...
        public System.Collections.Generic.List<object> DropdownListForColor(string sql)
        {
            var dt = ExecuteDataTable(sql);
            var list = new System.Collections.Generic.List<object>();
            if (null != dt)
            {
                foreach (System.Data.DataRow item in dt.Rows)
                {
                    list.Add(new { id = item["Id"].ToString(), text = item["Name"].ToString(), color=item["Color"] });
                }
            }
            return list;
        }
        ...
        public string FilterArrayString(string[] strarray)
        {
            var str= "''";
            foreach( var data in strarray)
            {
                str += FilterString(data)+"','";
        ...
        public Shared.Common.DbResponse ParseDbResponse(System.Data.DataTable dt)
        {

            var res = new Shared.Common.DbResponse();
            if (null == dt)
            {
                res.ErrorCode = 0;
                res.Message = "";
        ...
        public Shared.Common.DbResponse ParseDbResponse(string sql)
        {
